...
class ViewController: UIViewController {
    ...
    func fetchMiniAppMetaData(miniAppInfo: MiniAppInfo) {
        MiniApp.shared(with: Config.getCurrentConfig()).getMiniAppManifest(miniAppId: miniAppInfo.id, miniAppVersion: miniAppInfo.version.versionId) { (result) in
            switch result {
    ...
    func compareMiniAppMetaData(miniAppInfo: MiniAppInfo, completionHandler: @escaping (Result<Bool, Error>) -> Void) {
        guard let downloadedManifest = MiniApp.shared(with: Config.getCurrentConfig()).getDownloadedManifest(miniAppId: miniAppInfo.id) else {
