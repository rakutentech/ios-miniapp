...
class ViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "DisplayMiniApp" {
            guard let miniAppDisplay = currentMiniAppView else {
                displayAlert(title: MASDKLocale.localize("miniapp_sdk_ios_error_title"), message: MASDKLocale.localize("miniapp_sdk_ios_error_message_miniapp"), dismissController: true)
                return
            }
        ...
        if isMiniAppLaunchedAlready(key: miniAppInfo.id) {
            compareMiniAppMetaData(miniAppInfo: miniAppInfo) { (result) in
                ...
                case .failure(let error):
                    self.displayAlert(title: MASDKLocale.localize("miniapp_sdk_ios_error_title"), message: error.localizedDescription, dismissController: true)
                }
            }
        } else {
    ...
    func fetchMiniAppMetaData(miniAppInfo: MiniAppInfo) {
        MiniApp.shared(with: Config.current()).getMiniAppManifest(miniAppId: miniAppInfo.id, miniAppVersion: miniAppInfo.version.versionId) { (result) in
            ...
            case .failure:
                self.displayAlert(title: MASDKLocale.localize("miniapp_sdk_ios_error_title"), message: MASDKLocale.localize("miniapp_sdk_ios_error_message_single"), dismissController: true)
            }
        }
    }
